...
static void InitSelf() {
  GameModel mod = GameModel(Model(), 1, "Player");
  GameObject obj;

  mod.bounds = SBoundingBox(-.5,-.5,-.5);
   mod.bounds = SBoundingBox(-0.5, -0.5, -0.5);
   mod.bounds.update(0.5, 0.5, 0.5);
  obj = GameObject(mod.bounds, 0, vec3(0.0f), "Player");
...
void InitGeom() {
   ...
   lightx = lighty = 10.0f;
   scaleObj(3,0.5,0.5,0.5f);
   //rotateObj(3,0.0f,0.0f,90.0f);
   transObj(3,0.0,5.5,0.0f);
   Objects[3].setVelocity(vec3(0.5,0.5,0.5));
}
...
void Update(double timeStep) {
  for (int k = 0;k < Objects.size();k++) {
    cout << "boundingbox bottom: " << Objects[k].bounds.bottom << endl;
    cout << "boundingbox top: " << Objects[k].bounds.top << endl;
    cout << "boundingbox right: " << Objects[k].bounds.right << endl;
    cout << "boundingbox left: " << Objects[k].bounds.left << endl;
    cout << "boundingbox front: " << Objects[k].bounds.front << endl;
    cout << "boundingbox back: " << Objects[k].bounds.back << endl;
    cout << "Object name "<< Objects[k].name << endl;
  }
...
int main( int argc, char *argv[] )
{
   ...
   while (!glfwWindowShouldClose(window)) {
      double time = glfwGetTime(), lastTime = glfwGetTime();
      int width, height;
