...
void Update(double timeStep) {
  ...
  for (int i = dels.size() - 1; i >= 0; i--) {
     transObj(dels[i], -50000.0,-50000.0,-50000.0);
     //Objects.erase(Objects.begin() + dels[i]);
  }
}
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
  ...
  if (action == GLFW_PRESS || action == GLFW_REPEAT) {
     switch( key ) {
      ...
      case GLFW_KEY_W:
        transObj(0,(wBar*speed).x,0.0,(wBar*speed).z);
        eyePos += vec3(1.0,0.0,1.0)*wBar*speed;
        break;
      case GLFW_KEY_S:
        transObj(0,-(wBar*speed).x,0.0,-(wBar*speed).z);
        eyePos -= vec3(1.0,0.0,1.0)*wBar*speed;
        break;
      case GLFW_KEY_A:
        transObj(0,(uBar*speed).x,0.0,(uBar*speed).z);
        eyePos += vec3(1.0,0.0,1.0)*uBar*speed;
        break;
      case GLFW_KEY_D:
        transObj(0,-(uBar*speed).x,0.0,-(uBar*speed).z);
        eyePos -= vec3(1.0,0.0,1.0)*uBar*speed;
...
int main( int argc, char *argv[] )
{
   ...
   while (!glfwWindowShouldClose(window)) {
      double time1 = glfwGetTime(), lastTime = glfwGetTime();
      int width, height;
      glfwGetFramebufferSize(window, &width, &height);
      time1 = glfwGetTime();
      Update(1.01*(time1) -lastTime);
